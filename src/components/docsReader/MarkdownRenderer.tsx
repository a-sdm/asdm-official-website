...
const MermaidDiagram: React.FC<{ content: string }> = ({ content }) => {
  ...
  return (
    <div className="my-6 bg-gray-900 p-6 rounded-lg shadow-xl overflow-auto border border-yellow-500/30 w-full">
...
export default function MarkdownRenderer({ content }: MarkdownRendererProps) {
  ...
  return (
    <div className="prose prose-base max-w-none prose-invert w-full overflow-hidden">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          h1: ({ children }) => (
            <h1 className="text-2xl font-bold text-white mb-5 pb-2 border-b-4 border-yellow-400 shadow-lg break-words">
              {children}
            </h1>
          ),
          h2: ({ children }) => (
            <h2 className="text-xl font-semibold text-yellow-400 mt-6 mb-3 border-l-4 border-transparent hover:border-yellow-400 pl-2 break-words">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-lg font-semibold text-white mt-5 mb-2 break-words">
              {children}
            </h3>
          ),
          p: ({ children }) => (
            <p className="text-gray-300 text-sm leading-relaxed mb-3 break-words">
              {children}
            </p>
          ),
          ul: ({ children }) => (
          ...
          blockquote: ({ children }) => (
            <blockquote className="border-l-8 border-yellow-400 pl-4 py-2 my-3 bg-yellow-400/10 shadow-lg w-full overflow-hidden">
              <div className="text-gray-300 italic text-sm break-words">
                {children}
              </div>
            </blockquote>
          ),
          code: ({ className, children, ...props }: any) => {
            ...
            return !props.inline && match ? (
              ...
              <code className="bg-gray-800 text-yellow-400 px-1 py-0.5 text-xs shadow-md break-all" {...props}>
                {children}
              </code>
            );
          },
          table: ({ children }) => (
            <div className="overflow-x-auto my-3 w-full">
              <table className="w-full divide-y divide-gray-700 border-2 border-gray-700 shadow-lg hover:border-yellow-400/50">
                {children}
              </table>
            </div>
          ),
          ...
          td: ({ children }) => (
            <td className="px-3 py-2 text-xs text-gray-300 border-b border-gray-800 hover:bg-gray-800/50 break-words">
              {children}
            </td>
          ),
        }}
